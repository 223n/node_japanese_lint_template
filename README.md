...
npm install --save-dev \
  github:223n/node_japanese_lint_template#v2.1.1 \
  textlint markdownlint-cli2
```

`#v2.1.1`の部分は、実在するタグを指す必要がある。
...
{
  "devDependencies": {
    "@223n/lint-config-ja": "github:223n/node_japanese_lint_template#v2.1.1"
  }
}
```
...
{
  "devDependencies": {
    "@223n/lint-config-ja": "^2.1.1"
  }
}
```

git参照はタグで一点を指すだけなので、修正版が出ても自動では入らない。
`^2.1.1`と書ければ、後方互換のある修正は`npm update`で入る。
...
| 経路 | 利用側に要るもの | 版の指定 |
| ---- | ---- | ---- |
| npm | 依存を1行書くだけ | `^2.1.1`の範囲指定が使える |
| git参照 | 依存を1行書くだけ | タグで一点を指す |
| GitHub Packages | `.npmrc`とアクセストークン | `^2.1.1`の範囲指定が使える |
...
{
  "devDependencies": {
    "@223n/lint-config-ja": "git+ssh://git@github.com/223n/node_japanese_lint_template.git#v2.1.1"
  }
}
```
...
供給経路を見る道具（Socketなど）にかけると、依存の木に対する警告が並ぶ。
「難読化コード」のような強い文言も出るため、先に断っておく。
...
| レジストリ以外を出所とする依存 | 0 |
| 既知の脆弱性（`npm audit`） | 0 |
| コピーレフトのライセンス | 0（MIT 285、BSD 5、ISC 3、Apache-2.0 2） |

開発時の依存としての実害は「入れた瞬間に走るもの」と「検査を走らせたときに動くもの」に限られるが、
どちらも成立しない。

いちばん重い「難読化コード」は、縮小して配る成果物に対する誤検出である。
該当ファイルには元のソースを含むソースマップが同梱されており、意図を隠す目的とは正反対である。
しかもそのファイルはNodeでは読み込まれない。

警告ごとの内訳、唯一の注意点、手元で同じことを確かめる手順は
[依存の警告](https://github.com/223n/node_japanese_lint_template/blob/main/docs/%E4%BE%9D%E5%AD%98%E3%81%AE%E8%AD%A6%E5%91%8A.md)にある。
